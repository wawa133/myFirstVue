 ...
 * @LastEditTime: 2024-05-02 21:58:30
 * @LastEditors: liutq
 * @Reference: 
-->
<template>
...
import LeftLeader from "@/components/Notification/LeftLeader.vue";
import Notification from "@/components/Notification/NotificationMain.vue";
import AdminNotification from "@/components/Notification/adminNotification.vue";
import AdminmainBody from "@/components/mainBodyList/AdminmainBodyList.vue";

const userStore = useUserStore();
const layTableItenCurrent = ref("1");
const mainCallOption = ref({ callBack: null });
let layTableIndex = 1;
...
const addTab = function (option: any) {
  ...
  if (!haveItem) {
    ...
    arrLayTable.value.push({
      ...
      option: mainCallOption.value,
    } as { id: string; title: string; action: string; option: { callBack?: any }; closable: boolean });
...
const refreshMain = function (option: any) {
  mainCallOption.value.callBack = option.loadData;
  //console.log("option", option.loadData, mainCallOption.value.callBack);
};
</script>
